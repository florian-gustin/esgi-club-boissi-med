...
public class SearchContract {
    ...
    private static final Logger logger = LoggerFactory.getLogger(SearchContract.class);
    ...
    public SearchContract(CacheService cacheService, ContractInMemoryRepository contractInMemoryRepository, ObjectMapper objectMapper, ContractMapper contractMapper) {
    ...
    public ContractResponse execute(String subscriptionId) throws JsonProcessingException {
        var optionalContract = contractInMemoryRepository.findAll().stream().filter(c -> c.getSubscriptionId().equals(subscriptionId)).findFirst();

        if(optionalContract.isPresent()){
            optionalContract.get().setStatus("inactive");
            var response = new ContractResponse();
            response.setContractId(optionalContract.get().getContractId());
            response.setContractType(optionalContract.get().getContractType());
            response.setStatus(optionalContract.get().getStatus());
            return response;
        }
        return null;
